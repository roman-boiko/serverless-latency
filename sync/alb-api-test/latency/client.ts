...
export const lambdaHandler = async (event: any): Promise<any> => {
    ...
    const { body } = await request('http://' + url, {
        method: 'GET',
        query: { clientTime: clientTime.toISOString(), coldStart: coldStart },
    });
    const response = await body.json();
    const responseTime = new Date();
    const serverResponseTime = new Date(response.responseTime);
    const responseLatency = responseTime.getTime() - serverResponseTime.getTime();
